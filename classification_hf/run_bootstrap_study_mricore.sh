#!/usr/bin/env bash
# Bootstrap study with mricore backbone configs.
#
# Usage (from SpineFoundation/):
#   bash classification_hf/run_bootstrap_study_mricore.sh [n_runs] [n_bootstrap] [n_parallel]
#
...
CONFIGS=(
    "$ROOT/classification_hf/configs/rsna_nfn_mricore.yaml"
    "$ROOT/classification_hf/configs/rsna_scs_mricore.yaml"
    "$ROOT/classification_hf/configs/rsna_ss_mricore.yaml"
)
OUTPUT_DIRS=(
    "$ROOT/outputs_cls/rsna_nfn_mricore"
    "$ROOT/outputs_cls/rsna_scs_mricore"
    "$ROOT/outputs_cls/rsna_ss_mricore"
)
TASKS=("nfn" "scs" "ss")
LOG_DIR="$ROOT/classification_hf/logs_mricore"

cd "$ROOT"

echo "================================================================"
echo "Bootstrap study (mricore backbone) — $(date)"
